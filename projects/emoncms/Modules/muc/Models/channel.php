...
class Channel
{
	...
	public function create($userid, $ctrlid, $deviceid, $configs)
	{
		...
		$settings = $this->create_log_settings($userid, $nodeid, $id, $authorization, null);
		if ($description !== '') {
	...
	private function parse_channel($id, $description, $auth, $logsettings, $configs)
	{
		$channel = array(
				'id' => $id
		);
	...
	private function create_log_settings($userid, $nodeid, $id, $auth, $authid)
	{
		$settings = array(
				'nodeid' => $nodeid
		);
