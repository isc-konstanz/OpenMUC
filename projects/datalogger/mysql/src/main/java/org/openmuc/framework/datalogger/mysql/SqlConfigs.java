...
public class SqlConfigs extends Channel {
    ...
    protected void onConfigure() throws ArgumentSyntaxException {
    	if (host == null) {
    		host = SqlLogger.DB_HOST;
    	}
    	if (port == null) {
    		port = Integer.valueOf(SqlLogger.DB_PORT);
    	}
    	if (database == null) {
    	...
    	if (table == null) {
        	...
        	if (table.equals(valid)) {
        		throw new ArgumentSyntaxException("Table name invalid. Only alphanumeric letters separated by underscore are allowed: " + valid);
        	}
    	}
