...
public class Valve implements Runnable, RecordListener {
	...
	public Valve(int rotateDuration,
		...
		logger.debug("Configured position angle watcher interval of {}ms", positionAngleWatchInterval);
	}
	...
	public final void set(Value value) {
		...
		logger.debug("Received position angle setpoint value: {}", value.asDouble());
	...
	private void startWatcher() {
		...
		positionAngleWatcher.setName("TH-E EMS Heat Pump environment position angle watcher");
	...
	public void run() {
		...
		while (!deactivate) {
			try {
				...
				if (Double.isFinite(positionAngleRecord.getValue().asDouble()) && positionAngle != null &&
						positionAngle.getLatestRecord().getFlag() == Flag.VALID &&
