...
To install OpenMUC just download the link:https://www.openmuc.org/openmuc/download/[latest version]
and unpack it to your favourite destination.

OpenMUC requires Java 7 or higher, therefore make sure it is installed on your machine.

=== Start the Demo

The OpenMUC demo contains a simple application which demonstrates how you can
access channels and their records from an application. The application reads
data from channels of
the CSV driver, calculates new values from them and writes them to
other channels. The application can be used as starting point to create your
own OpenMUC application.

Open a terminal and navigate to the framework folder _(<your-path>/openmuc/framework)_ 
...
all the bundles located in the "bundle" folder. After initialization of the OSGi framework
you should be able to see the output of the demo application.

----
...
2019-02-18 17:33:00.011 INFO SimpleDemoApp - home1: current grid power = -4.672 kW
2019-02-18 17:33:05.006 INFO SimpleDemoApp - home1: current grid power = -4.666 kW
2019-02-18 17:33:10.007 INFO SimpleDemoApp - home1: current grid power = -4.671 kW
...
----

Among the bundles that are started is the Apache Gogo shell.
...
START LEVEL 1
   ID|State      |Level|Name
    ...
    ...
    7|Active     |    1|Logback Core Module (1.2.3)|1.2.3
    8|Active     |    1|OpenMUC App - Simple Demo (0.17.2)|0.17.2
    9|Active     |    1|OpenMUC Core - API (0.17.2)|0.17.2
   10|Active     |    1|OpenMUC Core - Data Manager (0.17.2)|0.17.2
   11|Active     |    1|OpenMUC Core - SPI (0.17.2)|0.17.2
   12|Active     |    1|OpenMUC Data Logger - ASCII (0.17.2)|0.17.2
   13|Active     |    1|OpenMUC Data Logger - SlotsDB (0.17.2)|0.17.2
   14|Active     |    1|OpenMUC Driver - CSV (0.17.2)|0.17.2
   15|Active     |    1|OpenMUC Server - RESTful Web Service (0.17.2)|0.17.2
   16|Active     |    1|OpenMUC WebUI - Base (0.17.2)|0.17.2
   17|Active     |    1|OpenMUC WebUI - Channel Access Tool (0.17.2)|0.17.2
   ...
...
The last WebUI plugin we want to look at is a customised visualisation for our demo application.
Click on _Applications_ and select __Simple Demo Visualisation__. The purpose of this plugin is
to show how OpenMUC channels can be accessed and used for individual visualisations. Detailed
informations about the development of such a plugin can be found in the
<<anchor-tutorial-webui-plugin,Tutorial Develop a Customised WebUI Plugin>>.
...
data for short storage and retrieval times. OpenMUC currently includes four data
loggers. The ASCII data logger saves data in a human readable text
format while SlotsDB saves data in a more efficient binary format. And
two loggers for remote system logging with AMQP or MQTT.
...
|loggingEvent| no | _boolean_ |false | If true, immediately logs latest record on value change. Only supported by some data loggers. See <<anchor-dataloggers, data logger description>> for more information.
...
In example 3 listening instead of sampling is used.
This means that the driver reports a new channel value to the
data manager when the value has changed for example.
...
IMPORTANT: When listening is true and additional a sampling interval is
defined then the sampling is ignored.
...
Example 5 extends example 3 by an additional event logging.
It loggs only if only a new value was received. (the logger needs to support evvent logging)
...
This runs OpenMUC as a background process.
If you want to run OpenMUC in the foreground run:
...
include::sections/driver/modbus-driver.txt[]

include::sections/driver/mbus-driver.txt[]

include::sections/driver/60870-driver.txt[]

include::sections/driver/61850-driver.txt[]

include::sections/driver/62056-driver.txt[]

include::sections/driver/dlms-driver.txt[]

include::sections/driver/knx-driver.txt[]

include::sections/driver/ehz-driver.txt[]

include::sections/driver/snmp-driver.txt[]

include::sections/driver/csv-driver.txt[]

include::sections/driver/aggregator-driver.txt[]

include::sections/driver/rest-driver.txt[]

include::sections/driver/amqp-driver.txt[]

[[anchor-dataloggers]]
== Dataloggers

include::sections/datalogger/ascii-logger.txt[]

include::sections/datalogger/amqp-logger.txt[]

include::sections/datalogger/mqtt-logger.txt[]

include::sections/datalogger/slotsdb-logger.txt[]

include::sections/datalogger/sql-logger.txt[]

[[anchor-libraries]]
== Libraries

include::sections/libraries/amqp.txt[]

include::sections/libraries/parser-service.txt[]

include::sections/libraries/ssl.txt[]
...
Plugin to show current values of selected channels.
Provides possibility to set values.
...
You can include your own Plugins in the OpenMUC WebUI by creating a java class that extends the WebUiPluginService.
This class also has to be annotated as a component.
The two functions getAlias and getName have to be overridden.
The alias is used to identify the plugin while the name will be displayed in the WebUI.
...
First you need a svg and assign all the svg Elements a unique id.
You can include an external svg as an Object Element or you can create the svg directly in html.
Either way the Element containing the svg needs an id which you can then use to access the svg in javascript.
...
Aside from changing what is displayed you can also manipulate the css in this way.
The following example would change the displayed texts color to blue.
...
Typically this functionality is needed when your browser sends an OPTIONS request instead of the request (f.e. GET, POST etc.) that you inteded to send.
Per default CORS is is deactivated for the rest Server.
The fuctionality can be activatet via the system.properties file located in the openmuc/framework/conf/ directory.
...
Multiple origins are semicolon seperated with the methods and headers seperated by the same order as the origins.
The methods and headers for each origin are then separated by comma.
This means that in the above example the origin:

http://localhost:4200 is a trusted origin which is allowed to send GET and PUT requests and uses the headers Autorization and Content-Type. +
http://localhost:8456 is a trusted origin which is allowed to send POST requests and uses the header Autorization.
